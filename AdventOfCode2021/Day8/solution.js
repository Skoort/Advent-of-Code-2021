...
{
    const inOutPair = x.split(' | ');
    return {
        in: inOutPair[0].split(' '),
        out: inOutPair[1].split(' ')
    }
});
...
const uniquelyIdentifiableLengths = [2, 3, 4, 7];

let numUniquelyIdentifiableLengths = 0;
for (let i = 0; i < lines.length; ++i)
{
    let line = lines[i];
    for (let j = 0; j < line.out.length; ++j)
    {
        if (uniquelyIdentifiableLengths.findIndex(x => x == line.out[j].length) != -1)
        {
            ++numUniquelyIdentifiableLengths;
        }
    }
}

console.log('Part 1:', numUniquelyIdentifiableLengths);
